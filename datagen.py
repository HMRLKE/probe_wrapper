...
def generate_dataset(ud_data_dict, morph_data_path_triplet, pos_tag, morph_tag, random=False):
    ...
    for morph_path in morph_data_path_triplet:
        #extended data
        ext_output_file = os.path.join("output_ext", morph_path)
        rnd_output_file = os.path.join("output_rnd", morph_path)

        ext_output_dir = os.path.dirname(ext_output_file)
        rnd_output_dir = os.path.dirname(rnd_output_file)
        ...
        if data_to_write and match_rate > 5:
            ...
            create_dir_if_needed(ext_output_dir)
            create_dir_if_needed(rnd_output_dir)
...
"""
def generate_joined_dataset(ud_data_paths, morph_data_paths, pos_tag, morph_tag, language):

    ud_data_dict  = extract_UD_sentences_from_all_files(ud_data_paths)
    collected_data = {"train":{}, "dev":{}, "test":{}}

    for morph_path in morph_data_paths:
        input_base = os.path.basename(morph_path)
        file_name = os.path.splittext(input_base)
        found_data = process_sentence_data(ud_data_dict, morph_path, pos_tag, morph_tag)
        collected_data[file_name].update(found_data)
        print(f"Collected the morphological probe data from: {morph_path}")

    for data_type in ["train", "dev", "test"]:
        output_file = os.path.join("output", "joined", pos_tag.lower() + "_" + morph_tag.lower(), language, f"{data_type}.txt")
        output_dir = os.path.dirname(output_file)
        # create output directory if does not exist
        create_dir_if_needed(output_dir)
        write_sentence_data(output_file, collected_data[data_type])
"""
...
def generate_data(tags: str, random = False):
    ...
    for lang, datalist in dataset_dict.items():
        ...
        for morph_pos, morph_data_paths in datalist:
            ...
            print(f"\nLanguage: {lang}; POS: {pos_tag}; MORPH: {morph_tag}")
...
def parse_arguments():
    ...
    parser.add_argument("--repo", type=str, default="probing")
    parser.add_argument("--tags", type=str, default=None)
    #parser.add_argument("--join", action='store_true', help='Enable the flag')
    parser.add_argument("--download", action='store_true', help='Only download git repositories.')
...
def main():
    ...
    # For an older version these variables can be used
    # UD_TREEBANK_NAME = "ud-treebanks-v2.9"
    # base_url = "https://lindat.mff.cuni.cz/repository/xmlui/bitstream/handle/11234/1-4611"

    base_url = "https://lindat.mff.cuni.cz/repository/xmlui/bitstream/handle/11234/1-5150"
    ud_treebank_name = "ud-treebanks-v2.12"
    #for probing only the English is available
    download_ud_treebank_if_needed(base_url, ud_treebank_name)

    language = parser.language
    morph_repo_name = parser.repo
    tags = parser.tags

    #join_lang_data = parser.join

    git_url = "https://github.com/juditacs/probing"
    download_git_repo_if_needed(git_url, "probing")
    git_url = "https://github.com/juditacs/morphology-probes"
    download_git_repo_if_needed(git_url, "morphology_probes")
    ...
    if morph_repo_name == "probing":
        ud_data_paths = list(conllu_file_path_loader(ud_treebank_name, "English"))
        morph_data_paths = list(morpho_file_path_loader(morph_repo_name))
        morph_tag, pos_tag = list(tags.split("_"))
        ud_data_dict  = extract_UD_sentences_from_all_files(ud_data_paths)
        generate_dataset(ud_data_dict, morph_data_paths, pos_tag, morph_tag)

    elif morph_repo_name == "morphology_probes":
        # The language is one iteration above to save a large ammount of repetitive CoNLLU file parsing
        # (multiple tags are used often with the same language)


        #for tag_list, lang in find_tag_combinations(tags, language):
        dataset_dict  = extended_morpho_dataset_paths(tags)
        for lang, datalist in dataset_dict.items():


            ud_data_paths = list(conllu_file_path_loader(ud_treebank_name, lang))
            ud_data_dict  = extract_UD_sentences_from_all_files(ud_data_paths)
        ...
        raise ValueError(f"Error: Wrong git repository name for the morphological probing data: {morph_repo_name}")
